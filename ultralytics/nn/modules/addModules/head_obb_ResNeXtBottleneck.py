...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.export and self.format in ("tflite", "edgetpu"):
            ...
            img_size = torch.tensor(
                [img_w, img_h, img_w, img_h], device=box.device
            ).reshape(1, 4, 1)
    ...
    def decode_bboxes(self, bboxes):
        ...
        return (
            dist2bbox(self.dfl(bboxes), self.anchors.unsqueeze(0), xywh=True, dim=1)
            * self.strides
        )
...
class OBB_SEResNeXt(Detect):
    ...
    def decode_bboxes(self, bboxes):
        ...
        return (
            dist2rbox(self.dfl(bboxes), self.angle, self.anchors.unsqueeze(0), dim=1)
            * self.strides
        )
